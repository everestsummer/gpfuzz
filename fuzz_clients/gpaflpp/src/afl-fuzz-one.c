...
extern bool has_something_for_reloading;
extern bool four_same_class;
extern int new_seed_count;
/* MOpt */
...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (unlikely(afl->shm.cmplog_mode &&
    ...
    afl->stage_short = "reload";
    afl->stage_name = "reloading!";
    ...
    afl->stage_max   =  50; //The testcases in the folder.
    afl->stage_cur   =  0;
   //afl->stage_cur_val = 0;
    ...
    while(in_file = readdir(dir)) {
        ...
        if(fp) {
            ...
            if (common_fuzz_stuff(afl, buf_file, buf_len)) {
                goto skip_to_next;
            }
    ...
    afl->stage_finds[STAGE_RELOAD]  += new_hit_cnt - orig_hit_cnt;
    afl->stage_cycles[STAGE_RELOAD] += afl->stage_max;
...
#define MUTATE_ASCII_DICT 64
  ...
  //TODO: if we've got something inside reloading, set stage_cur to stage_max - 1.

  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {

    if(has_something_for_reloading){
       printf("Havoc: Force reloading. \n");
       afl->stage_cur = afl->stage_max - 1;  //Make it stop soon.
    }
