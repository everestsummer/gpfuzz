...
bool server_responsed = true;
bool reload_processed = true;
...
void *fuzzer_side_socket(void* arg) {
  ...
  while(1){
    ...
    if(strncmp(buffer, "identify_result\nclass_id:", 25) == 0) {
      ...
      //if(current_identify_result != -1)
      server_responsed = true;

      //no matter what, set reload_proc = false;
      reload_processed = false;
...
char g_path_buffer[65535];

void to_absolute_path(const char *relative_path, char *absolute_path) {
    char current_dir[PATH_MAX];
    if (getcwd(current_dir, sizeof(current_dir)) != NULL) {
        snprintf(absolute_path, 65535, "%s/%s", current_dir, relative_path);
    } else {
        perror("getcwd() error");
    }
}
  ...
  if (fault == crash_mode) {
    ...
    if (out_dir[0] == '/') {
      fn = alloc_printf("%s/queue/id:%06u,%s", out_dir, queued_paths,
		...
		to_absolute_path(out_dir, g_path_buffer);
		fn = alloc_printf("%s/queue/id:%06u,%s", g_path_buffer, queued_paths,
                      describe_op(hnb));
    ...
    if (out_dir[0] == '/') {
	  ...
	  to_absolute_path(out_dir, g_path_buffer);
      fn = alloc_printf("%s/queue/id_%06u", g_path_buffer, queued_paths);
	}
#endif /* ^!SIMPLE_FILES */


   // AFL doesn't support MAIN/SUB mode so comment this.
   // if(afl->is_secondary_node){  //this shall only send by main node.
   //    printf("Skip: secondary mode\n");
   //    goto skip_socket;
   // }
     #define REDUCED_FILE_COUNT 25
    //Cares only totally new cov.
    //if(hnb != 2)
    //  goto skip_socket;

    if(new_seed_count >= 50 && !reload_processed && !server_responsed)   
      goto skip_socket;

    if(!four_same_class && new_seed_count < 50){
      recent_seed_list[new_seed_count] = alloc_printf("%s|", fn);
    ...
    if((new_seed_count >= 50 || (half_the_testcase && new_seed_count >= REDUCED_FILE_COUNT)) && !four_same_class){
      //Prepare data
    ...
    if (four_same_class && (new_seed_count >= 50 || (half_the_testcase && new_seed_count >= 20))){
...
//    reload_processed = true;
    ...
    stage_short = "reload";
    stage_name = "reloading!";

    if(four_same_class)
      new_seed_count = 50; //force sending.

    stage_val_type = STAGE_VAL_NONE;

    has_something_for_reloading = false;
    ...
    stage_max   =  50; //The testcases in the folder.
    stage_cur   =  0;
   //stage_cur_val = 0;

    const char* reload_dir = "./reload";

    dir = opendir(reload_dir);
    if(!dir){
        goto skip_to_next;
    }
    ...
    while(in_file = readdir(dir)) {
        ...
        snprintf(path, sizeof(path) - 1, "%s/%s", reload_dir, in_file->d_name);
        fp = fopen(path, "rb");
        //printf("reloading %s\n", path);
        if(fp) {
            filelist[file_idx++] = strdup(path);

            fseek(fp, 0, SEEK_END);
            buf_len = ftell(fp);
            fseek(fp, 0, SEEK_SET);

            buf_file = malloc(buf_len);
            fread(buf_file, sizeof(char), buf_len, fp);
            fclose(fp);

            if (common_fuzz_stuff(argv, buf_file, buf_len)) {
                reload_processed = true;
                goto skip_to_next;
            }
    ...
    //Remove processed seeds from reloading folder.
    for (int x = 0; x < file_idx; x++) {
        if(filelist[x]){
          unlink(filelist[x]);
          free(filelist[x]);
        }
    ...
    stage_finds[STAGE_RELOAD]  += new_hit_cnt - orig_hit_cnt;
    stage_cycles[STAGE_RELOAD] += stage_max;
...
skip_to_next:
	...
	//Note: EcoFuzz goto havoc directly, so we must add relead before this goto
	//
    goto havoc_stage;
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:H:o:f:m:t:T:dnCB:S:M:x:Q")) > 0)

    switch (opt) {
      case 'H':
        printf("Halves the testcase!\n");
        half_the_testcase = true;
        break;
      case 'i': /* input dir */
  ...
  char* reload_dir = alloc_printf("%s/../reload/", out_dir);
  mkdir(reload_dir, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  ck_free(reload_dir);
