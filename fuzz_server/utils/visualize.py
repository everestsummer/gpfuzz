...
def plot_encoded_data(X_encoded, y_predicted, y_true, N_classes, title="", output="clustering", X_raw=None,
                      show_images=False, verbosity=1, subset=None, prototypes=None, devs=None):
    plt.cla()
    plt.clf()
    plt.close()
    ...
    for j, c in enumerate(classes_used):
        ...
        #    plt.set_cmap('binary')
        #    img_size = 0.04
        #    ax.imshow(image_to_show, aspect='auto', extent=(
        #    means[j, 0] - size0 * img_size, means[j, 0] + size0 * img_size, means[j, 1] - size1 * img_size,
        #    means[j, 1] + size1 * img_size), zorder=-1)

        #    rect = patches.Rectangle((means[j, 0] - size0 * img_size, means[j, 1] - size1 * img_size),
        #                             2 * size0 * img_size, 2 * size1 * img_size, linewidth=1, edgecolor=colors[j],
        #                             facecolor='none')
        #    ax.add_patch(rect)
        ...
        if ((prototypes is not None) and (devs is not None)):
            ...
            e = patches.Ellipse(xy=(prototypes_reduced[j, 0], prototypes_reduced[j, 1]), width=major, height=minor,
                                angle=majora / 2.0 / np.pi * 360.0, color=colors[j], alpha=0.1)
            ax.add_patch(e)

        if not show_images:
            handles_true.append(
                ax.scatter(X_encoded_reduced[inds_true, 0], X_encoded_reduced[inds_true, 1], facecolors=colors[j],
                           edgecolors=colors[j], label=str(classes_used[j]), marker="o", alpha=1.0))

        #if show_images:
            #for i in inds_true:
                #plt.set_cmap('binary')
                #img_size = 0.03
                #ax.imshow(X_raw[i, :], aspect='auto', extent=(
                #X_encoded_reduced[i, 0] - size0 * img_size, X_encoded_reduced[i, 0] + size0 * img_size,
                #X_encoded_reduced[i, 1] - size1 * img_size, X_encoded_reduced[i, 1] + size1 * img_size), zorder=-1)
                #rect = patches.Rectangle(
                #    (X_encoded_reduced[i, 0] - size0 * img_size, X_encoded_reduced[i, 1] - size1 * img_size),
                #    2 * size0 * img_size, 2 * size1 * img_size, linewidth=1, edgecolor=colors[j], facecolor='none')
                #ax.add_patch(rect)
    ...
    for j, c in enumerate(classes_used):
        ...
        if not show_images:
            handles_pred.append(
                ax.scatter(X_encoded_reduced[inds_pred, 0], X_encoded_reduced[inds_pred, 1], facecolors=colors[j],
                           edgecolors=colors[j], label=str(classes_used[j]), marker="o", alpha=1.0))
